    ...
    (
      ...
      $scope.saveDisplayName = function saveDisplayName() {
        ...
        if ($scope.newDisplayName.value && $scope.newDisplayName.value.length >= 5) {
          const currentDisplayName = $scope.exchange.displayName;
          $scope.exchange.displayName = $scope.newDisplayName.value;

          return EmailPro
            .updatingDisplayName($scope.exchange.domain, $scope.newDisplayName.value)
            .then(() => {
            ...
            .catch((err) => {
              $scope.exchange.displayName = currentDisplayName;
