angular.module("Module.emailpro.controllers").controller("EmailProUpdateRenewCtrl", ($scope, $stateParams, EmailPro, $location) => {
    "use strict";
    $scope.exchange = $scope.currentActionData;
    ...
    var checkForChanges = function () {
        ...
        $scope.bufferedAccounts.list.results
            .forEach((buffer, index) => {
                ...
                _.remove($scope.buffer.changes, { primaryEmailAddress: buffer.primaryEmailAddress });
    ...
    $scope.checkboxStateChange = function (state, value) {
        if ($scope.bufferedAccounts && $scope.bufferedAccounts.list && $scope.buffer.ids && $scope.bufferedAccounts.list.results) {
