angular.module("Module.emailpro.controllers").controller("EmailProTabAccountsCtrl", ($scope, EmailPro, $stateParams) => {
    ...
    $scope.addAccountOptionIsNotAvailable = function () {
        return $scope.exchange.nicType.indexOf(EmailPro.nicAdmin) === -1 && $scope.exchange.nicType.indexOf(EmailPro.nicTech) === -1;
    };
    ...
    $scope.isEditable = function (account) {
        return (account.state === $scope.stateOk || account.state === $scope.stateTaskDoing || account.state === $scope.stateTaskError) && !$scope.noDomainFlag;
    };
