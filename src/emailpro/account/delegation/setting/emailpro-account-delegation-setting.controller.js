angular.module("Module.emailpro.controllers").controller("EmailProDelegationSettingsCtrl", ["$scope", "EmailPro", "$timeout", function ($scope, EmailPro, $timeout) {
    "use strict";
    ...
    $scope.getDelegationRight = function (count, offset) {
        ...
        EmailPro.getDelegationRight($scope.selectedAccount.primaryEmailAddress, count, offset, $scope.form.search)
            .then((accounts) => {
                $scope.loading = false;
                $scope.bufferAccounts = accounts; // keep the original data as a reference point to compare changes
    ...
    $scope.updateDelegationRight = function () {
        ...
        EmailPro.updateDelegationRight(getChanges()
        ).then((data) => {
