    ...
    .controller("EmailProAddAccountCtrl", ($scope, $stateParams, EmailPro, EmailProPassword) => {
        ...
        $scope.setPasswordsFlag = function (selectedAccount) {
            ...
            if (selectedAccount.password.length > 0) {
                ...
                if (!$scope.simplePasswordFlag && selectedAccount.password !== selectedAccount.passwordConfirmation) {
        ...
        $scope.loadAccountOptions = function () {
            EmailPro.getNewAccountOptions($stateParams.productId).then((data) => {
                ...
                if (data.availableDomains.length === 0) {
                    ...
                    $scope.accountToAdd.completeDomain = data.availableDomains[0];
                    $scope.accountToAdd.accountLicense = data.availableTypes[0];
        ...
        // $scope.setPasswordsFlag($scope.accountToAdd);
            if (!$scope.valid.legalWarning) {
                return false;
            } else if ($scope.simplePasswordFlag || $scope.differentPasswordFlag || $scope.containsNameFlag) {
                return false;
            } else if (!$scope.accountToAdd.completeDomain || $scope.accountToAdd.completeDomain.name === undefined) {
                return false;
            } else if (!$scope.accountToAdd.login) {
                return false;
            } else if (!$scope.accountToAdd.password ||
                $scope.accountToAdd.password.indexOf(" ") > -1 ||
                $scope.accountToAdd.password !== $scope.accountToAdd.passwordConfirmation) {
                return false;
            }
            return EmailProPassword.passwordSimpleCheck($scope.accountToAdd.password, false, $scope.newAccountOptions.minPasswordLength);
