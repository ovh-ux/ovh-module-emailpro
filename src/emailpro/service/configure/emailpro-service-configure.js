angular
  ...
  .controller(
    ...
    (
      ...
      function getDetails() {
        return emailproConfigure
          .fetchingDetails(exchange.domain)
          .then((serviceDescription) => {
            $scope.service = serviceDescription;
            $scope.service.lockoutThreshold = _.isNumber($scope.service.lockoutThreshold)
              ? $scope.service.lockoutThreshold : 0;

            $scope.service.minPasswordLength = _.isNumber($scope.service.minPasswordLength)
              ? $scope.service.minPasswordLength : 0;
            $scope.service.minPasswordAge = _.isNumber($scope.service.minPasswordAge)
              ? $scope.service.minPasswordAge : 0;
            $scope.service.maxPasswordAge = _.isNumber($scope.service.maxPasswordAge)
      ...
      $scope.putDetails = function putDetails() {
        ...
        return emailproConfigure
          .updatingDetails(exchange.domain, dataToSend)
          .then(() => {
            $scope.resetAction();
  ...
  .controller(
    ...
    function EmailProServicesConfigureFormCtrl($scope) {
      const self = this;
      const intRegex = /^\d+$/;
